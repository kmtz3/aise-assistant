...
| **Active Package** | `Current Account Owner = ["<user-uuid>"]` **and** `Customer (M:N)` = linked Customer URL(s) **and** `Active for (1:N)` = linked Customer URL(s) **only if** `Start Date ≤ today ≤ End Date`, otherwise leave empty | `Customer (M:N)` is the permanent link — always set. `Active for (1:N)` is the live-ledger link — set only when the package is currently active by date. `Current Account Owner` mirrors Customer.Owner; set explicitly on create before Resync fires. |
