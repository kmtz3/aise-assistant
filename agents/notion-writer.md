...
| **Active Package** | `Current Account Owner = ["<user-uuid>"]` | Mirror of Customer.Owner. Set explicitly on create — the Resync button hasn't fired yet. |
...
- **Tasks**: create only for actions assigned to the user (PB-side). Customer-side actions do NOT go in the Tasks DB. **Always set `Customers`** (Productboard customer record for internal tasks). **Always set `Owner` (= the user as creator) and `Current Account Owner` (= the user) on create.**
- **Sessions**: set `Delivered By` to the actual presenter(s) — the user for her own deliveries, predecessor AISE for backfilled historical sessions, leave blank + flag if unknown. Leave `Current Account Owner` blank on create — the automation fills it. Never default historical sessions to the user as Delivered By if she didn't actually deliver them.
- **Active Package**: `Current Account Owner` mirrors `Customer.Owner`. Set on create; let the Resync button maintain afterwards. **Never write to the rollup `Delivered By (Sessions)`** — it's auto-computed.
...
- **Prep content**: goes inside a collapsible toggle heading (`📋 Prep — YYYY-MM-DD`) on the Session page, never replacing existing notes.
...
- **Status = Done ≠ inactive on Active Packages.** Means contracted services are delivered; the user still runs recurring syncs in post-services rhythm. Don't flag as a contradiction unless `Active? = NO` and there's no upcoming sync cadence.
