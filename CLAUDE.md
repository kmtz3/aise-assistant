...
| Command | Purpose |
|---|---|
| `/customer-setup <customer>` | Set up a newly assigned or inherited account — researches company, pulls Gong + Gmail history from previous owners, finds the right Master Package, proposes and writes Customer page update + Active Package with history summary. Run before `/customer-plan-engagement`. |
...
| `/customer-plan-next <customer>` | Plan next 2–4 sessions, flag dependencies and risks. |
| `/customer-plan-engagement <customer>` | Full program plan for a newly assigned (or restructured) customer — goals, milestones, phases, A/E/S sessions. Lands in the Active Package page in Notion. |
