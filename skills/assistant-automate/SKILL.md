...
Read the procedure in `agents/workflow-advisor.md` and execute it inline as the main assistant — do not try to spawn `workflow-advisor` as a subagent (custom agents in this plugin are procedure documents, not registered subagent types). The steps:
