...
argument-hint: "[--date YYYY-MM-DD] [--open] [--no-blocks]"
---

Generate a daily briefing page for $ARGUMENTS (defaults to today).

Read the procedure in [`agents/daily-brief.md`](../agents/daily-brief.md) and execute it inline as the main assistant — do not try to spawn `daily-brief` as a subagent (custom agents in this plugin are procedure documents, not registered subagent types). The steps:
